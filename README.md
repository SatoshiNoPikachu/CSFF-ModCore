...
- CardTag 扩展方法 GetCards 已弃用，由只读集合接口类型的扩展属性 Cards 代替。
- CardTypes 扩展方法 GetCards 已弃用，由只读集合接口类型的扩展属性 Cards 代替。
- 修复蓝图主分组未成功添加到游戏中的问题。
...
- 加载器修改功能增强与变更：
  - 新增 $matchTargets 支持，用于指定额外的修改目标。
  - 修改的源目标现在始终会被修改，即便它不满足匹配条件。
- Loader.FixData 方法现在会忽略元字段。
- CardExtraData 适配游戏新的存档数据结构。
...
- 加载器修改功能增强：
  - 支持字段类型为对象的修改模式（WarpType: 5）。
  - GSM 和 DOM 现在支持 $override 块，用于对象覆写功能。
  - 恢复支持多个匹配修改，且匹配目标不会被重复修改。
  - 异步并发优化：
    - 不再并发读取 JSON 文件。
    - 并发修改匹配到的多个目标。
- 修复加载的 UniqueIDScriptable 对象未被排序的问题。
- 现在模组服务在注册模组时会输出日志。
...
- 新增衍生插件 ModCore.Games，用于实现针对游戏的模块功能：
  - 为模组核心提供将所加载的生存指南（帮助）添加到游戏中的功能。
  - 新增数据类型 MC-BlueprintTab：
    - 为模组核心提供将所加载的蓝图主分组添加到游戏蓝图面板中的功能。
    - 支持根据角色特质控制蓝图主分组的显示。
  - 实现蓝图主分组按钮的翻页控制，该功能从模组“增加手牌槽位”中移植并优化，请确保将该模组同步更新以避免冲突。
  - 实现卡牌动作按钮组的滚动视图，该功能从模组“增加手牌槽位”中移植并优化，请确保将该模组同步更新以避免冲突。
- 模组命名空间现在不允许包含符号“|”，因为这与多态对象引用语法冲突。
- 模组数据类新增静态方法 GetNamespace，用于获取数据键中的命名空间。
- 模组数据类现在包含由加载器提供的内部数据对象表，并提供 GetData 方法进行访问。
- 模组服务的 GetMods 方法已标记为弃用，由只读集合接口类型的 Mods 属性代替。
- 实用工具新增 Unity 相关扩展成员：
  - 新增只写属性 Transform.LocalX。
  - 新增只写属性 Transform.LocalY。
  - 新增只写属性 RectTransform.Width。
  - 新增只写属性 RectTransform.Height。
- 现在 Game.Gm 和 Game.Grm 被声明为可空。
- 修复 Game.Grm 始终为空的问题。
- 移除 Game.PreInitOneShotEvent 事件。
...
- 加载器优化与修复：
  - 通过缓存 IList 泛型接口类型参数和游戏类型判断结果优化性能。
  - 修复加载数组时会覆盖数组已有成员的问题。
- 修复可能导致协程工作不正常的方法补丁。
...
- 针对 CSFF 的版本：目标框架降级至 .net48，并重新实现因此失效的代码，UI 预制件 LogPopup 暂时移除。
- 现在预加载场景时将会销毁场景内物体。
